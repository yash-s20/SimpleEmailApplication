...
{
	if(argc != 6) {
		cerr<<"Usage: "<<argv[0]<<" <serverIPAddr>:<port> <user-name> <passwd> <file1,file2...> <local folder>\n";
		exit(1);
	...
	while(token!=NULL) {
		...
		try {
			...
			{
				int dummytemp = stoi(temp.substr(i, temp.length() - i));
			}
	...
	for (int i = 0; i < noOfFiles; ++i)
	{
		...
		if (bytes_received==0 or bytes_received==-1) {
			// this is where the file number sent is not present in database
			close(sockfd);
			exit(0);
		}
